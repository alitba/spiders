...
def get_url(url,user,password):
    ...
    wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'body > div.geetest_panel.geetest_wind > div.geetest_panel_box.geetest_no_logo.geetest_panelshowslide > div.geetest_panel_next > div > div.geetest_wrap > div.geetest_slider.geetest_ready > div.geetest_slider_button')))
    user_input = browser.find_element_by_id('login-username')
    pwd_input = browser.find_element_by_id('login-passwd')
    btn = browser.find_element_by_css_selector('#geetest-wrap > ul > li.btn-box > a.btn.btn-login')
...
# 4.获取滑动验证图片在浏览器的位置，使用location是获取标签左上角的位置，然后再通过该标签的大小，即可算出其四个角的位置。
...
if __name__ == '__main__':
    browser = get_url('https://passport.bilibili.com/login', '18521300298', 'fengye---111')  # 此函数的定义在第3点
    time.sleep(1)
